...
for text, expected in test_cases:
    ...
    safe_print(f"{status}: '{text}' -> Expected: {expected}, Got: {detected}")
...
for text, target_lang, should_have_mismatch in fixer_tests:
    ...
    status = "PASS" if passed else "FAIL"
    safe_print(f"{status}: '{text}' (target: {target_lang}) -> Expected mismatch: {should_have_mismatch}, Got: {has_mismatch}")

    if has_mismatch:
        fixed = enforce_language(text, target_lang)
        safe_print(f"  Fixed: '{fixed}'")
